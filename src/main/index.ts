...
import {
  ...
  updateArsivKlasorTanim,
  getArsivDusunceTanimlari,
  addArsivDusunceTanim,
  deleteArsivDusunceTanim,
  syncArsivDefinitions,
  getSetting,
  setSetting,
  getAllSettings,
  // E-APOSTİL
  ...
  addArsivBatch, // YENİ IMPORT
  addEnvanterMalzemeBatch,
  ...
  deleteGorevlendirme,
  // GÖNDERİLEN BELGELER
  getGonderilenBelgeler,
  addGonderilenBelge,
  updateGonderilenBelge,
  deleteGonderilenBelge,
  // NOTLAR
  getNotlar,
  addNot,
  updateNot,
  ...
  updateBelge,
  deleteBelge,
  getBelgelerKategoriler,
  addBelgeKategori,
  updateBelgeKategori,
  deleteBelgeKategori,
  getBelgeDogrulamaSiteleri,
  addBelgeDogrulamaSitesi,
  updateBelgeDogrulamaSitesi,
  deleteBelgeDogrulamaSitesi,
  getKurumHarcamalar,
  addKurumHarcama,
  updateKurumHarcama,
  deleteKurumHarcama,
  getKurumHarcamaKategoriler,
  addKurumHarcamaKategori,
  deleteKurumHarcamaKategori
} from './database'
import { initFirebaseAuth } from './firebaseConfig'
import {
  initYabanciTemsilciliklerDB,
  getTemsilcilikler,
  addTemsilcilik,
  updateTemsilcilik,
  deleteTemsilcilik,
  reorderTemsilcilikler
} from './db/yabanci_temsilcilikler'
import {
  initTurkTemsilcilikleriDB,
  getTurkTemsilcilikler,
  addTurkTemsilcilik,
  updateTurkTemsilcilik,
  deleteTurkTemsilcilik,
  reorderTurkTemsilcilikler
} from './db/turk_temsilcilikleri'
...
app.whenReady().then(async () => {
  ...
  const GH_TOKEN = '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'
  ...
  // Firebase Anonim Oturum Aç (Firestore erişimi için gerekli)
  ...
  ipcMain.handle('add-arsiv-batch', async (_, d) => addArsivBatch(d)) // YENİ HANDLER
  ...
  ipcMain.handle('add-arsiv-tanim', async (_, data) => addArsivKlasorTanim(data))
  ipcMain.handle('delete-arsiv-tanim', async (_, id) => deleteArsivKlasorTanim(id))
  ipcMain.handle('update-arsiv-tanim', async (_, data) => updateArsivKlasorTanim(data))

  // Arşiv Düşünceler
  ipcMain.handle('get-arsiv-dusunce-tanimlari', async () => getArsivDusunceTanimlari())
  ipcMain.handle('add-arsiv-dusunce-tanim', async (_, aciklama) => addArsivDusunceTanim(aciklama))
  ipcMain.handle('delete-arsiv-dusunce-tanim', async (_, id) => deleteArsivDusunceTanim(id))

  // Sync
  ipcMain.handle('sync-arsiv-definitions', async () => syncArsivDefinitions())
  ...
  ipcMain.handle('update-arsiv-imha-komisyonu', async (_, d) => updateArsivImhaKomisyonu(d))
  ipcMain.handle('delete-arsiv-imha-komisyonu', async (_, id) => deleteArsivImhaKomisyonu(id))

  // Yabancı Temsilcilikler
  ipcMain.handle('get-temsilcilikler', async () => getTemsilcilikler())
  ipcMain.handle('add-temsilcilik', async (_, data) => addTemsilcilik(data))
  ipcMain.handle('update-temsilcilik', async (_, data) => updateTemsilcilik(data))
  ipcMain.handle('delete-temsilcilik', async (_, id) => deleteTemsilcilik(id))
  ipcMain.handle('reorder-temsilcilikler', async (_, data) => reorderTemsilcilikler(data))

  // Türk Yurtdışı Temsilcilikleri
  ipcMain.handle('get-turk-temsilcilikler', async () => getTurkTemsilcilikler())
  ipcMain.handle('add-turk-temsilcilik', async (_, data) => addTurkTemsilcilik(data))
  ipcMain.handle('update-turk-temsilcilik', async (_, data) => updateTurkTemsilcilik(data))
  ipcMain.handle('delete-turk-temsilcilik', async (_, id) => deleteTurkTemsilcilik(id))
  ipcMain.handle('reorder-turk-temsilcilikler', async (_, data) => reorderTurkTemsilcilikler(data))
  ...
  ipcMain.handle('create-ek-excel', async (_, data) => {
    return new Promise((resolve) => {
      const scriptName = 'excel_ops.py'
      const scriptPath = is.dev
        ? join(__dirname, `../../resources/${scriptName}`)
        : join(process.resourcesPath, scriptName)
      ...
      // Masaüstü yolunu ekle
      data['desktop_path'] = app.getPath('desktop')
      data['resource_path'] = is.dev ? join(__dirname, '../../resources') : process.resourcesPath

      pythonProcess.stdin.write(JSON.stringify(data))
      pythonProcess.stdin.end()

      // UTF-8 encoding is handled by converting buffer to string based on default,
      // but usually explicit encoding in spawn options or handling buffer is needed for special chars.
      // spawn defaults to buffer.
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          console.error(`Python process exited with code ${code}`)
          console.error('Error details:', errorString)
          resolve({ success: false, error: errorString || 'Python script hata kodu ile kapandı' })
          return
        }

        try {
          // dataString might contain multiple chunks, ensure it is complete JSON
          const parsed = JSON.parse(dataString)
          resolve(parsed)
        } catch (e) {
          console.error('JSON Parse Hatası:', e)
          resolve({ success: false, error: 'JSON Parse Hatası: ' + dataString })
        }
      })
    })
  })
  ...
  ipcMain.handle('update-envanter-personel-tanim', async (_, v) => {
    // Fix lint error: missing return type
    return updateEnvanterPersonelTanim(v.id, v.ad)
  })

  // Envanter Import (SQLite)
  ipcMain.handle('get-db-tables', async (_, dbPath) => {
    return new Promise((resolve) => {
      const scriptName = 'db_ops.py'
      const scriptPath = is.dev
        ? join(__dirname, `../../resources/${scriptName}`)
        : join(process.resourcesPath, scriptName)
      ...
      const payload = JSON.stringify({ command: 'get_tables', db_path: dbPath })
      ...
      pythonProcess.on('close', () => {
        try {
          // dataString might be fragmented, but handled by buffer usually in simple scripts
          resolve(JSON.parse(dataString))
        } catch (e) {
          console.error('JSON Parse Error:', e)
          resolve({ success: false, error: 'JSON Parse Error: ' + dataString })
        }
      })
    })
  })

  ipcMain.handle('read-db-table', async (_, tableInfo) => {
    // tableInfo = { dbPath, tableName }
    return new Promise((resolve) => {
      const scriptName = 'db_ops.py'
      const scriptPath = is.dev
        ? join(__dirname, `../../resources/${scriptName}`)
      ...
      // JSON payload to python
      const payload = JSON.stringify({
        command: 'read_table',
        db_path: tableInfo.dbPath,
  ...
  ipcMain.handle('add-envanter-batch', async (_, materials) => addEnvanterMalzemeBatch(materials))

  // PERSONEL GÖREVLENDİRMELERİ
  ipcMain.handle('get-gorevlendirmeler', async () => getGorevlendirmeler())
  ipcMain.handle('add-gorevlendirme', async (_, data) => addGorevlendirme(data))
  ipcMain.handle('update-gorevlendirme', async (_, data) => updateGorevlendirme(data))
  ipcMain.handle('delete-gorevlendirme', async (_, id) => deleteGorevlendirme(id))

  // GÖNDERİLEN BELGELER
  ipcMain.handle('get-gonderilen-belgeler', async () => getGonderilenBelgeler())
  ipcMain.handle('add-gonderilen-belge', async (_, data) => addGonderilenBelge(data))
  ipcMain.handle('update-gonderilen-belge', async (_, data) => updateGonderilenBelge(data))
  ipcMain.handle('delete-gonderilen-belge', async (_, id) => deleteGonderilenBelge(id))

  // NOTLAR
  ipcMain.handle('get-notlar', async () => getNotlar())
  ipcMain.handle('add-not', async (_, data) => addNot(data))
  ipcMain.handle('update-not', async (_, data) => updateNot(data))
  ipcMain.handle('delete-not', async (_, id) => deleteNot(id))

  // BELGELER ARŞİVİ
  ipcMain.handle('get-belgeler', async () => getBelgeler())
  ipcMain.handle('add-belge', async (_, data) => addBelge(data))
  ipcMain.handle('update-belge', async (_, data) => updateBelge(data))
  ipcMain.handle('delete-belge', async (_, id) => deleteBelge(id))
  ipcMain.handle('get-belgeler-kategoriler', async () => getBelgelerKategoriler())
  ipcMain.handle('add-belge-kategori', async (_, ad) => addBelgeKategori(ad))
  ipcMain.handle('update-belge-kategori', async (_, data) => updateBelgeKategori(data))
  ipcMain.handle('delete-belge-kategori', async (_, id) => deleteBelgeKategori(id))

  ipcMain.handle('get-belge-dogrulama-siteleri', async () => getBelgeDogrulamaSiteleri())
  ipcMain.handle('add-belge-dogrulama-sitesi', async (_, data) => addBelgeDogrulamaSitesi(data))
  ipcMain.handle('update-belge-dogrulama-sitesi', async (_, data) => updateBelgeDogrulamaSitesi(data))
  ipcMain.handle('delete-belge-dogrulama-sitesi', async (_, id) => deleteBelgeDogrulamaSitesi(id))
  ipcMain.handle('open-url', async (_, url: string) => shell.openExternal(url))

  ipcMain.handle('get-kurum-harcamalar', async (_, params) => getKurumHarcamalar(params))
  ipcMain.handle('add-kurum-harcama', async (_, data) => addKurumHarcama(data))
  ipcMain.handle('update-kurum-harcama', async (_, data) => updateKurumHarcama(data))
  ipcMain.handle('delete-kurum-harcama', async (_, id) => deleteKurumHarcama(id))
  ipcMain.handle('get-kurum-harcama-kategoriler', async () => getKurumHarcamaKategoriler())
  ipcMain.handle('add-kurum-harcama-kategori', async (_, data) => addKurumHarcamaKategori(data))
  ipcMain.handle('delete-kurum-harcama-kategori', async (_, id) => deleteKurumHarcamaKategori(id))
  ...
  ipcMain.handle('scan-folder-pdfs', async (_, folderPath: string) => {
    const results: { ad: string; dosya_yolu: string; kategori: string }[] = []
    const scanDir = (dirPath: string, relativePart: string): void => {
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            results.push({
              ad: entry.name.replace(/\.pdf$/i, ''),
              dosya_yolu: fullPath,
              kategori: relativePart || 'Genel'
            })
          }
        }
      } catch {
    ...
    const rootName = folderPath.split(/[\\/]/).pop() || 'Genel'
    scanDir(folderPath, rootName)
    return results
  })
