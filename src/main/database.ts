...
// NOT: doc.data() içinde gömülü 'id' alanı varsa (eski SQLite göçü) Firestore string ID'sini
// ezmemek için data'dan 'id' alanını çıkarıyoruz.
const formatDoc = (doc: any): any => {
  const { id: _dataId, ...rest } = doc.data() || {}
...
export async function updateArsivDosya(data: any): Promise<boolean> {
  const { id, _mergedIds, sira: _sira, ...updateData } = data
  const docId = String(id)
  if (!docId || docId === 'undefined' || docId === 'null') {
    throw new Error('Geçersiz kayıt ID: ' + id)
  }
  await updateDoc(doc(db, 'arsiv_dosyalar', docId), updateData)
  return true
}

export async function deleteArsivDosya(id: string): Promise<boolean> {
  await deleteDoc(doc(db, 'arsiv_dosyalar', String(id)))
  return true
}
// --- ARŞİV TANIMLARI ---
...
export async function getGorevlendirmeler(): Promise<any[]> {
  const snapshot = await getDocs(collection(db, 'personel_gorevlendirme'))
  const docs: any[] = snapshot.docs.map((d) => {
    const { id: _dataId, ...rest } = d.data() || {}
    return { id: d.id, ...rest }
  })
  return docs.sort((a, b) =>
    String(b.gorevlendirme_tarihi || '').localeCompare(String(a.gorevlendirme_tarihi || ''))
  )
}

export async function addGorevlendirme(data: any): Promise<any> {
  const res = await addDoc(collection(db, 'personel_gorevlendirme'), data)
  return { id: res.id, ...data }
}

export async function updateGorevlendirme(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'personel_gorevlendirme', String(id)), rest)
}

export async function deleteGorevlendirme(id: string): Promise<void> {
  await deleteDoc(doc(db, 'personel_gorevlendirme', String(id)))
}

// --- GÖNDERİLEN BELGELER ---

export async function getGonderilenBelgeler(): Promise<any[]> {
  const snapshot = await getDocs(collection(db, 'gonderilen_belgeler'))
  const docs: any[] = snapshot.docs.map((d) => {
    const { id: _dataId, ...rest } = d.data() || {}
    return { id: d.id, ...rest }
  })
...
export async function addGonderilenBelge(data: any): Promise<any> {
  const res = await addDoc(collection(db, 'gonderilen_belgeler'), data)
  return { id: res.id, ...data }
}

export async function updateGonderilenBelge(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'gonderilen_belgeler', String(id)), rest)
}

export async function deleteGonderilenBelge(id: string): Promise<void> {
  await deleteDoc(doc(db, 'gonderilen_belgeler', String(id)))
}

// --- NOTLAR ---

export async function getNotlar(): Promise<any[]> {
  const snapshot = await getDocs(collection(db, 'notlar'))
  const docs = snapshot.docs.map(formatDoc)
  return docs.sort((a, b) => {
    const aDate = a.guncelleme_tarihi || a.tarih || ''
    const bDate = b.guncelleme_tarihi || b.tarih || ''
    return bDate.localeCompare(aDate)
  })
}

export async function addNot(data: any): Promise<any> {
  const res = await addDoc(collection(db, 'notlar'), data)
  return { id: res.id, ...data }
}

export async function updateNot(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'notlar', id), rest)
}

export async function deleteNot(id: string): Promise<void> {
  await deleteDoc(doc(db, 'notlar', id))
}

// --- BELGELER ARŞİVİ ---

export async function getBelgeler(): Promise<any[]> {
  const snap = await getDocs(query(collection(db, 'belgeler'), orderBy('olusturma_tarihi', 'desc')))
  return snap.docs.map(formatDoc)
}

export async function addBelge(data: any): Promise<string> {
  const now = new Date().toISOString()
  const docRef = await addDoc(collection(db, 'belgeler'), {
    ad: data.ad || '',
    aciklama: data.aciklama || '',
...
export async function updateBelge(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'belgeler', id), rest)
}

export async function deleteBelge(id: string): Promise<void> {
  await deleteDoc(doc(db, 'belgeler', id))
}

export async function getBelgelerKategoriler(): Promise<any[]> {
  const snap = await getDocs(query(collection(db, 'belgeler_kategoriler'), orderBy('ad', 'asc')))
  return snap.docs.map(formatDoc)
}

export async function addBelgeKategori(ad: string): Promise<string> {
  const docRef = await addDoc(collection(db, 'belgeler_kategoriler'), { ad })
  return docRef.id
}

export async function updateBelgeKategori(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'belgeler_kategoriler', id), rest)
}

export async function deleteBelgeKategori(id: string): Promise<void> {
  await deleteDoc(doc(db, 'belgeler_kategoriler', id))
}

// BELGE DOĞRULAMA SİTELERİ
export async function getBelgeDogrulamaSiteleri(): Promise<any[]> {
  const snap = await getDocs(query(collection(db, 'belge_dogrulama_siteleri'), orderBy('ulke', 'asc')))
  return snap.docs.map(formatDoc)
}

export async function addBelgeDogrulamaSitesi(data: any): Promise<string> {
  const docRef = await addDoc(collection(db, 'belge_dogrulama_siteleri'), {
    ulke: data.ulke || '',
    bayrak: data.bayrak || '🌐',
    site_adi: data.site_adi || '',
    url: data.url || '',
    belge_turleri: data.belge_turleri || '',
    aciklama: data.aciklama || '',
...
export async function updateBelgeDogrulamaSitesi(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'belge_dogrulama_siteleri', id), rest)
}

export async function deleteBelgeDogrulamaSitesi(id: string): Promise<void> {
  await deleteDoc(doc(db, 'belge_dogrulama_siteleri', id))
}

// KURUM HARCAMA
export async function getKurumHarcamalar(params?: { baslangic?: string; bitis?: string }): Promise<any[]> {
  const snap = await getDocs(collection(db, 'kurum_harcamalar'))
  const tumKayitlar = snap.docs.map(formatDoc)
  if (params?.baslangic && params?.bitis) {
    return tumKayitlar
      .filter(k => k.tarih >= params.baslangic! && k.tarih <= params.bitis!)
      .sort((a, b) => b.tarih.localeCompare(a.tarih))
  }
  return tumKayitlar.sort((a, b) => b.tarih.localeCompare(a.tarih))
}

export async function addKurumHarcama(data: any): Promise<string> {
  const docRef = await addDoc(collection(db, 'kurum_harcamalar'), {
    baslik: data.baslik || '',
...
export async function updateKurumHarcama(data: any): Promise<void> {
  const { id, ...rest } = data
  await updateDoc(doc(db, 'kurum_harcamalar', id), {
    baslik: rest.baslik || '',
    tutar: Number(rest.tutar) || 0,
...
export async function deleteKurumHarcama(id: string): Promise<void> {
  await deleteDoc(doc(db, 'kurum_harcamalar', id))
}

export async function getKurumHarcamaKategoriler(): Promise<any[]> {
  const snap = await getDocs(query(collection(db, 'kurum_harcama_kategoriler'), orderBy('ad', 'asc')))
  return snap.docs.map(formatDoc)
}

export async function addKurumHarcamaKategori(data: any): Promise<string> {
  const docRef = await addDoc(collection(db, 'kurum_harcama_kategoriler'), {
    ad: data.ad || '',
    tur: data.tur || 'HER_IKISI'
  })
  return docRef.id
}

export async function deleteKurumHarcamaKategori(id: string): Promise<void> {
  await deleteDoc(doc(db, 'kurum_harcama_kategoriler', id))
}
