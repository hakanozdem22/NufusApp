import { useEffect, useState, useMemo, FormEvent, ReactElement } from 'react'
...
export const EnvanterDashboard = (): ReactElement => {
  ...
  // View mode: flat table or grouped by personel
  const [viewMode, setViewMode] = useState<'table' | 'grouped'>('table')
  const [collapsedGroups, setCollapsedGroups] = useState<Set<string>>(new Set())

  const groupedByPersonel = useMemo(() => {
    const groups = new Map<string, EnvanterMalzeme[]>()
    filteredItems.forEach((item) => {
      const key = item.personel?.trim() || 'Atanmamış'
  ...
  const toggleGroup = (personel: string): void => {
    setCollapsedGroups((prev) => {
      const next = new Set(prev)
      if (next.has(personel)) next.delete(personel)
      else next.add(personel)
      return next
    })
  }
  ...
  const handlePrint = async (mode: 'zimmet' | 'marka' | 'detay'): Promise<void> => {
    try {
      showToast('PDF hazırlanıyor...', 'warning')
      const titleMap = {
        zimmet: 'ENVANTER ZİMMET RAPORU',
        marka: 'MARKAYA GÖRE ENVANTER LİSTESİ',
        detay: 'ENVANTER DETAY LİSTESİ',
      }
      await window.api.createPdfEnvanter({
        liste: filteredItems,
        title: titleMap[mode],
        summary: mode === 'zimmet',
        group_by_marka: mode === 'marka',
      })
    ...
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-gray-950/50 transition-colors duration-500">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 p-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                >
                  <FileText size={16} strokeWidth={2.5} /> Zimmet Rapor
                </button>
                <button
                  onClick={() => handlePrint('marka')}
                  className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-xs font-black text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all uppercase tracking-tighter"
                >
                  <FileText size={16} strokeWidth={2.5} /> Marka Raporu
                </button>
                <button
                  onClick={() => handlePrint('detay')}
      ...
      <div className="max-w-[1600px] mx-auto w-full px-8 pt-8">
        <div className="bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl p-4 rounded-[2.5rem] border border-white dark:border-gray-800 shadow-xl flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="flex items-center gap-1 p-1 bg-slate-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shrink-0">
              <button
                onClick={() => setViewMode('table')}
                title="Tablo Görünümü"
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all ${viewMode === 'table' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
              >
                <List size={14} strokeWidth={2.5} /> Tablo
              </button>
              <button
                onClick={() => setViewMode('grouped')}
                title="Sorumluya Göre Grupla"
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all ${viewMode === 'grouped' ? 'bg-white dark:bg-gray-700 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
              >
                <Users size={14} strokeWidth={2.5} /> Sorumlu
              </button>
            </div>
      ...
      <div className="flex-1 max-w-[1600px] mx-auto w-full px-8 py-8 relative min-h-0">
        ...
        {viewMode === 'table' ? (
                ...
                <tbody className="divide-y divide-gray-100/50 dark:divide-gray-800/50">
                  {loading ? (
                    ...
                    filteredItems.map((item) => (
                      <tr key={item.id} className="group hover:bg-slate-50/50 dark:hover:bg-gray-800/30 transition-all duration-300">
                        ...
                        <td className="px-8 py-5">
                          <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-lg text-[10px] font-black uppercase tracking-tighter">{item.kategori}</span>
                        </td>
                        ...
                        <td className="px-8 py-5">
                          <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${item.durum === 'Sağlam' ? 'bg-green-100/50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border border-green-200/50' : item.durum === 'Arızalı' ? 'bg-amber-100/50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border border-amber-200/50' : 'bg-red-100/50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200/50'}`}>
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all duration-300">
                            <button onClick={() => handleEdit(item)} className="w-9 h-9 flex items-center justify-center text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all shadow-sm hover:shadow-lg hover:shadow-blue-500/20" title="Düzenle">
                              <Edit size={16} strokeWidth={2.5} />
                            </button>
                            <button onClick={() => handleDeleteClick(item.id!)} className="w-9 h-9 flex items-center justify-center text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-all shadow-sm hover:shadow-lg hover:shadow-red-500/20" title="Sil">
          ...
          /* ── GROUPED BY PERSONEL VIEW ── */
          <div className="overflow-auto h-full space-y-4 scrollbar-hide pb-4">
            {loading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="animate-pulse h-24 bg-white/80 dark:bg-gray-900/80 rounded-[2rem] border border-gray-100 dark:border-gray-800"></div>
              ))
            ) : filteredItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 opacity-30 py-24">
                <Archive size={48} strokeWidth={1} />
                <p className="font-bold text-gray-500 uppercase tracking-widest">Kayıtlı malzeme bulunamadı</p>
              </div>
            ) : (
              Array.from(groupedByPersonel.entries()).map(([personel, personelItems]) => {
                const isCollapsed = collapsedGroups.has(personel)
                const totalAdet = personelItems.reduce((s, i) => s + (i.adet || 0), 0)
                const isAtanmamis = personel === 'Atanmamış'
                return (
                  <div key={personel} className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden">
                    {/* Group Header */}
                    <button
                      onClick={() => toggleGroup(personel)}
                      className="w-full flex items-center justify-between px-8 py-5 hover:bg-slate-50/50 dark:hover:bg-gray-800/30 transition-all group"
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${isAtanmamis ? 'bg-gray-100 dark:bg-gray-800 text-gray-400' : 'bg-purple-600 text-white shadow-purple-500/20'}`}>
                          <User size={22} strokeWidth={2.5} />
                        </div>
                        <div className="text-left">
                          <p className={`font-black text-base tracking-tight ${isAtanmamis ? 'text-gray-400 dark:text-gray-500 italic' : 'text-gray-800 dark:text-gray-100'}`}>{personel}</p>
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.15em] mt-0.5">Zimmet Sorumlusu</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                          <div className="flex flex-col items-center px-5 py-2 bg-slate-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                            <span className="text-lg font-black text-purple-600 dark:text-purple-400">{personelItems.length}</span>
                            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Kalem</span>
                          </div>
                          <div className="flex flex-col items-center px-5 py-2 bg-slate-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
                            <span className="text-lg font-black text-blue-600 dark:text-blue-400">{totalAdet}</span>
                            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Toplam Adet</span>
                          </div>
                        </div>
                        <ChevronDown
                          size={20}
                          ...
                          <table className="w-full text-left border-collapse min-w-[860px]">
                            <thead>
                              <tr className="bg-slate-50/80 dark:bg-gray-800/60">
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 w-[25%]">Malzeme Bilgisi</th>
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Kategori</th>
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 text-center">Adet</th>
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Konum</th>
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700">Durum</th>
                                <th className="px-8 py-3.5 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-100 dark:border-gray-700 text-right">Eylem</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100/50 dark:divide-gray-800/50">
            ...
            <form onSubmit={handleSubmit} className="p-10 overflow-y-auto max-h-[75vh] scrollbar-hide space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    list="yer-list-form"
                    placeholder="Depo / Oda seçiniz veya yazınız"
                    className="w-full bg-slate-50 dark:bg-gray-800 border border-transparent focus:border-blue-500 focus:bg-white dark:focus:bg-gray-900 rounded-2xl px-6 py-4 text-sm font-bold outline-none ring-4 ring-transparent focus:ring-blue-500/10 transition-all dark:text-white shadow-inner"
                    value={formData.konum}
                    onChange={(e) => setFormData({ ...formData, konum: e.target.value })}
                  />
                  <datalist id="yer-list-form">
                    {yerler.map((y) => (
                      <option key={y.id} value={y.yer_adi} />
                    ))}
                  </datalist>
                </div>
                <div className="space-y-2">
