...
export const EnvanterSettings = () => {
  ...
  const [yerler, setYerler] = useState<any[]>([])
  const [personeller, setPersoneller] = useState<any[]>([]) // Personel tanımları (Harcama/Envanter ortak olabilir ama burada ayrı tutuyoruz)
  ...
  // HANDLERS
  const addKategori = async (text: string) => {
    await window.api.addEnvanterKategori(text)
    fetchData()
  }
  const delKategori = async (id: string | number) => {
    await window.api.deleteEnvanterKategori(id)
    fetchData()
  }

  const addYer = async (text: string) => {
    await window.api.addEnvanterYer(text)
    fetchData()
  }
  const delYer = async (id: string | number) => {
    await window.api.deleteEnvanterYer(id)
    fetchData()
  }

  const addPersonel = async (text: string) => {
    await window.api.addEnvanterPersonelTanim(text)
    fetchData()
  }
  const delPersonel = async (id: string | number) => {
    await window.api.deleteEnvanterPersonelTanim(id)
    fetchData()
  }
