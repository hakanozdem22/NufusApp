...
export const useTebdilViewModel = () => {
  ...
  const dosyaEkle = async () => {
    ...
    try {
      if (window.api) {
        ...
        const data = await window.api.getTebdilFiles(form.id)
        setDosyalar(data)
        const eklenenTip = yeniDosya.tip
        setYeniDosya({ ad: '', yol: '', tip: 'DOSYA' })
        mesajGoster('Eklendi.')

        // Yerel listeyi hemen güncelle (IKONUN HEMEN YEŞİL OLMASI İÇİN)
        setListe((prev) =>
          prev.map((item) => {
            if (String(item.id) === String(form.id)) {
              const field = eklenenTip === 'LINK' ? 'link_sayisi' : 'dosya_sayisi'
              return { ...item, [field]: (Number(item[field]) || 0) + 1 }
            }
            return item
          })
        )

        // Veritabanından çekerek garantiye al (Hafif gecikmeli ki database kendine gelsin)
        setTimeout(() => verileriGetir(), 1000)
      }
    } catch (e) {
      mesajGoster('Hata.', 'hata')
    }
  ...
  const dosyaSil = async (id: number) => {
    if (window.api) {
      // Hangi dosyanın silindiğini tipini bulalım (State'den bulabiliriz)
      const silinen = dosyalar.find((d) => String(d.id) === String(id))

      await window.api.deleteTebdilFile(id)
      const data = await window.api.getTebdilFiles(form.id)
      setDosyalar(data)

      // Yerel listeyi hemen güncelle
      if (silinen) {
        setListe((prev) =>
          prev.map((item) => {
            if (String(item.id) === String(form.id)) {
              const field = silinen.tip === 'LINK' ? 'link_sayisi' : 'dosya_sayisi'
              return { ...item, [field]: Math.max(0, (Number(item[field]) || 0) - 1) }
            }
            return item
          })
        )
  ...
  const kaydet = async () => {
    ...
    try {
      if (window.api) {
        const payload = { ...form }
        // Eğer yeni ise ve konvansiyon yoksa, aktifTab'ı ekle
        if (!payload.id && (!payload.konvansiyon || payload.konvansiyon.length === 0)) {
        ...
        if (form.id) {
          const { dosya_sayisi, ...updatePayload } = payload
          await window.api.updateTebdil(updatePayload)
  ...
  const tabiTemizle = async () => {
    const silinecekUlkeler = filtrelenenler
    if (silinecekUlkeler.length === 0) return mesajGoster('Temizlenecek öğe yok.', 'hata')

    const onay = confirm(
      `${aktifTab === 'IKILI_ANLASMA' ? 'İkili Anlaşma' : aktifTab === 'VIYANA_1968' ? '1968 Viyana' : '1949 Cenevre'} bölümündeki TÜM kartlar (${silinecekUlkeler.length} adet) silinecek. Emin misiniz?`
    )
    ...
    try {
      if (window.api) {
        for (const ulke of silinecekUlkeler) {
          await window.api.deleteTebdil(ulke.id)
        }
        await verileriGetir()
        mesajGoster('Bölüm temizlendi.')
      }
    } catch (e) {
      console.error(e)
      mesajGoster('Temizleme sırasında hata oluştu.', 'hata')
    } finally {
      setYukleniyor(false)
    }
  }
  ...
  const duzenle = (item: any) => {
    ...
    // Geriye dönük uyumluluk: Eğer konvansiyon tekil string gelirse diziye çevir
    let convArray = item.konvansiyon
    if (typeof convArray === 'string') convArray = [convArray]
    if (!convArray) convArray = []

    setForm({ ...item, site_url: parsedSites, konvansiyon: convArray })
    setModalAcik(true)
  }
  ...
  const filtrelenenler = liste.filter((item) => {
    ...
    // Geriye dönük uyumluluk ve dizi kontrolü
    const conv = item.konvansiyon
    let matchConvention = false

    if (Array.isArray(conv)) {
      matchConvention = conv.includes(aktifTab)
    } else if (typeof conv === 'string') {
      matchConvention = conv === aktifTab
    } else if (!conv && aktifTab === 'VIYANA_1968') {
      matchConvention = true // Eski boş veriler varsayılan 1968
    }

    return nameMatch && matchConvention
  })

  const hizliUlkeEkle = async (ulkeAdi: string, bayrakKodu: string) => {
    try {
      if (window.api) {
        const payload = {
          ulke_adi: ulkeAdi,
          bayrak_url: bayrakKodu,
          aciklama: '',
          site_url: '[]',
          konvansiyon: [aktifTab]
        }
