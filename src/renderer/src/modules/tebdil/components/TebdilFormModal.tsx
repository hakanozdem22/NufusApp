              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group transition-colors">
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="w-full text-left space-y-4 mt-2">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">
                        Konvansiyon / Kategori (Birden Fazla Seçilebilir)
                      </label>
                      <div className="flex p-1 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl gap-1 transition-colors">
                        {[
                          { id: 'VIYANA_1968', label: '1968' },
                          { id: 'CENEVRE_1949', label: '1949' },
                          { id: 'IKILI_ANLASMA', label: 'İkili' }
                        ].map((t) => {
                          const isSelected =
                            Array.isArray(form.konvansiyon) && form.konvansiyon.includes(t.id)
                          return (
                            <button
                              key={t.id}
                              type="button"
                              onClick={() => {
                                const current = Array.isArray(form.konvansiyon)
                                  ? form.konvansiyon
                                  : []
                                let next: string[] = []
                                if (current.includes(t.id)) {
                                  next = current.filter((c) => c !== t.id)
                                } else {
                                  next = [...current, t.id]
                                }
                                setForm({ ...form, konvansiyon: next })
                              }}
