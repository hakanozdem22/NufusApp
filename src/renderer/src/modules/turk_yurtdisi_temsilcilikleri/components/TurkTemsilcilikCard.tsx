...
export const TurkTemsilcilikCard = ({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border-2 shadow-sm hover:shadow-xl transition-all group flex flex-col overflow-hidden relative min-h-[260px] border-transparent hover:border-red-400 dark:hover:border-red-500">
      {/* GÖRSEL ALANI */}
      <div className="h-28 bg-gray-50 dark:bg-gray-700/50 border-b dark:border-gray-700 relative flex items-center justify-center overflow-hidden p-3 shrink-0">
        {showImage ? (
          <img
            ...
            className="h-full w-auto object-contain drop-shadow-md group-hover:scale-105 transition-transform border border-gray-200 dark:border-gray-600 rounded"
          />
        ) : (
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar relative">
        {temsilcilikList.map((temsilcilik) => (
          <div
            key={temsilcilik.id}
            className="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2.5 border border-gray-100 dark:border-gray-600/50 hover:border-red-300 dark:hover:border-red-500/50 transition-colors relative group/item"
          >
            {/* Başlık (Şehir & Tipi) */}
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="text-[10px] font-bold text-red-600 dark:text-red-400 uppercase tracking-wider mt-0.5 ml-5">
                  {temsilcilik.tip}
                </div>
              </div>
            ...
            <div className="space-y-1 ml-5">
              {temsilcilik.telefon && (
                <div className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <span className="truncate" title={temsilcilik.telefon}>
                    {temsilcilik.telefon}
                  </span>
                </div>
              )}
              {temsilcilik.eposta && (
                <div className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <span className="truncate" title={temsilcilik.eposta}>
                    {temsilcilik.eposta}
                  </span>
                </div>
              )}
      ...
      <div className="p-2 border-t border-gray-100 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-800/30 shrink-0">
        <button
          ...
          className="w-full py-1.5 flex items-center justify-center gap-1.5 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50/50 dark:bg-red-900/20 border border-dashed border-red-200 dark:border-red-800 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 hover:border-red-300 dark:hover:border-red-700 transition-all"
        >
