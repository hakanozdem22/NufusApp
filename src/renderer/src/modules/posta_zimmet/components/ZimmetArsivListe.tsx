...
import { History, Search, Trash2, Calendar, FileCheck2, MapPin, CreditCard, Hash, QrCode, FileText } from 'lucide-react'
...
export const ZimmetArsivListe = ({
  ...
  return (
    <div className="flex flex-col bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-gray-100 dark:border-gray-800 shadow-xl overflow-hidden h-full relative group">
      {/* HEADER SECTION */}
      <div className="px-10 py-8 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-6 shrink-0 relative">
        ...
        <div className="relative flex-1 max-w-md w-full group/search">
          ...
          <input
            className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-gray-800 border-none rounded-lg text-sm font-bold outline-none ring-4 ring-transparent focus:ring-blue-500/10 focus:bg-white dark:focus:bg-gray-900 transition-all dark:text-white shadow-inner"
        ...
        <button
          onClick={onPrint}
          disabled={arsivListe.length === 0}
          className="flex items-center gap-2.5 px-5 py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-indigo-500/25 transition-all active:scale-95 shrink-0"
        >
          <FileText size={16} strokeWidth={2.5} /> PDF Oluştur
        </button>
      </div>
