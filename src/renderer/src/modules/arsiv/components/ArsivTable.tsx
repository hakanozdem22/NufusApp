        ...
        <table className="w-full text-left border-separate border-spacing-0 min-w-[1200px]">
          ...
          <tbody className="divide-y divide-gray-100/50 dark:divide-gray-800/50">
            {sortedListe.map((item, idx) => {
              ...
              const suresiiz =
                String(item.yili || '').trim() === '-' ||
                String(item.saklama_suresi || '').trim() === '-' ||
                String(item.aciklama || '').toLowerCase().includes('süresiz saklanır')
              const saklamaSuresi = parseInt(String(item.saklama_suresi || '0'))
              const yil = parseInt(String(item.yili || '0'))
              const buYil = new Date().getFullYear()
              const imhaZamani = yil + saklamaSuresi
              const imhaDurumu = suresiiz ? 'SÜRESİZ' : imhaZamani < buYil ? 'GİTMİŞ' : 'BEKLİYOR'
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-2 text-center">
                    {suresiiz ? (
                      <div className="inline-flex px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg font-black text-[9px] tracking-widest shadow-sm border border-indigo-100 dark:border-indigo-800/30">
                        SÜRESİZ
                      </div>
                    ) : (
                  ...
                  <td className="px-6 py-2">
                    <div className="flex flex-col gap-1">
                      {suresiiz ? (
                        <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest w-fit border bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-200/50 dark:border-indigo-900/30">
